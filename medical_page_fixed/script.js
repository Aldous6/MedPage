...
const menuCloseBtn = document.getElementById("menuCloseBtn") // <--- NUEVO SELECTOR
...
;(() => {
  ...
  // Modifica aquí tus items. 
  // Para YouTube usa type: "youtube" e inserta solo el ID.
  const MEDIA_ITEMS = [
    {
      type: "image",
    ...
    },
    {
      ...
      type: "youtube",
      id: "dN3LtDgNkzM", 
      src: "https://img.youtube.com/vi/dN3LtDgNkzM/hqdefault.jpg", 
  ...
  const AUTOPLAY_DELAY = 12000 // Aumentado a 6s para dar tiempo a cargar

  // --- FUNCIONES ---

  // Crea el elemento HTML (img, video local o youtube)
  function createMediaElement(item) {
    // --- 1. CASO YOUTUBE ---
    if (item.type === "youtube") {
      const iframe = document.createElement("iframe")
      const videoId = item.id
      // URL con parámetros para autoplay, mute (necesario para autoplay), loop y limpieza visual
      ...
      // Nota: El CSS debe tener .media-layer iframe { width: 100%; height: 100%; border: none; }
      return iframe
    }
  ...
  function updateMeta() {
    ...
    // Detectamos si es video (local o youtube)
    const isVideo = item.type === "video" || item.type === "youtube"

    if (mediaKind) mediaKind.textContent = isVideo ? "Video" : "Imagen"
  ...
  function renderThumbs() {
    ...
    MEDIA_ITEMS.forEach((item, idx) => {
      ...
      const img = document.createElement("img")
      // Lógica para saber qué imagen mostrar en el thumb
      const isVideo = item.type === "video" || item.type === "youtube"
      
      // Si es video/youtube usa el poster o src (asumiendo que src en youtube es la miniatura jpg)
      img.src = isVideo ? (item.poster || item.src) : item.src
      img.alt = ""
      img.loading = "lazy"
      btn.appendChild(img)

      // Si es cualquier tipo de video, ponemos el icono de Play
      if (isVideo) {
  ...
  function crossfadeTo(newIndex) {
    ...
    // 3. Limpieza de videos anteriores y Youtube IFRAMES
    const oldMedia = outgoing.querySelector("video, iframe")
    if (oldMedia) {
      setTimeout(() => { 
        try { 
          // Si es video HTML5, pausar
          if(oldMedia.tagName === 'VIDEO') oldMedia.pause();
          // Resetear src para cortar streaming (importante para YouTube)
          oldMedia.src = ""; 
        } catch(e){} 
      }, 600) 
    }
  ...
  // Si el mouse entra al carrusel, DETENEMOS el cambio automático
  stage.addEventListener("mouseenter", stopAutoplay)
  
  // Si el mouse sale del carrusel, REINICIAMOS el cambio automático
