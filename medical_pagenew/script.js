...
/* =============================
   ...
   - Luxury easing
   - Runs once on view
   - Handles integers + percent range like "20–30%"
   ============================= */
;(() => {
  ...
  function parseStat(text) {
    ...
    // Integer like "98", "11" (also allows commas)
 ...
 function animateNumber(el, spec) {
  ...
  // micro-settle config (muy sutil)
  const settleWindow = 0.88 // a partir de aquí empieza el settle (0..1)
  const settlePeak = 0.965  // punto donde alcanza el overshoot
  const settleEnd = 1.0     // vuelve al valor final al terminar

  function microSettle(t, baseValue, overshootValue) {
    // t in [0,1]
    if (t <= settleWindow) return baseValue

    // normaliza a [0,1] dentro de la ventana final
    const u = (t - settleWindow) / (settleEnd - settleWindow)

    // curva en 2 fases: sube al pico y baja al final
    // fase 1: 0..p  | fase 2: p..1
    const p = (settlePeak - settleWindow) / (settleEnd - settleWindow)

    if (u <= p) {
      const k = u / Math.max(1e-6, p) // 0..1
      return baseValue + (overshootValue - baseValue) * k
    } else {
      const k = (u - p) / Math.max(1e-6, (1 - p)) // 0..1
  ...
  function frame(now) {
    ...
    if (spec.kind === "number") {
      ...
      // overshoot: mínimo 1, máximo ~3% (cap) para que sea elegante
      ...
      // Animamos hacia el límite superior y hacemos micro-settle ahí,
      // luego al final hacemos snap a "20–30%"
      const to = spec.to
      const base = to * e
      const bump = Math.min(Math.max(1, Math.round(to * 0.05)), 6)
      const over = to + bump
    ...
    if (t < 1) {
      ...
      // Snap final exacto (respeta “20–30%”)
      el.textContent = spec.finalText
  ...
  // Store final text and reset to 0 for the effect
